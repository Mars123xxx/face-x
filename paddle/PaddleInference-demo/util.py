...
class MTCNN:
    def __init__(self):
        self.pnet = init_predictor('../output/Backbone_epoch49')
        self.rnet = init_predictor('../output/Backbone_epoch49')
        self.onet = init_predictor('../output/Backbone_epoch49')

    def predict_pnet(self, img):
        # 获取输入的名称
        input_names = self.pnet.get_input_names()
        # print(input_names)   # ['x']
        handle_image = self.pnet.get_input_handle(input_names[0])
        # 设置输入
        input_img_size = img.shape
        handle_image.reshape([1, 3, input_img_size[2], input_img_size[3]])
        handle_image.copy_from_cpu(img)
        # 运行predictor
        self.pnet.run()
        # 获取输出
        output_names = self.pnet.get_output_names()
        cls_prob = self.pnet.get_output_handle(output_names[0])
        bbox_pred = self.pnet.get_output_handle(output_names[1])
        cls_prob = cls_prob.copy_to_cpu()  # numpy.ndarray类型
    ...
    def predict_rnet(self, img):
        # 获取输入的名称
        input_names = self.rnet.get_input_names()
        handle_image = self.rnet.get_input_handle(input_names[0])
        # 设置输入
        input_img_size = img.shape
        ...
        self.rnet.run()
        # 获取输出
        output_names = self.rnet.get_output_names()
        cls_prob = self.rnet.get_output_handle(output_names[0])
        bbox_pred = self.rnet.get_output_handle(output_names[1])
        cls_prob = cls_prob.copy_to_cpu()  # numpy.ndarray类型
        bbox_pred = bbox_pred.copy_to_cpu()
    ...
    def predict_onet(self, img):
        ...
        # 获取输入的名称
        input_names = self.onet.get_input_names()
        handle_image = self.onet.get_input_handle(input_names[0])
        # 设置输入
        input_img_size = img.shape
        handle_image.reshape([input_img_size[0], 3, input_img_size[2], input_img_size[3]])
        handle_image.copy_from_cpu(img)
        ...
        self.onet.run()
        # 获取输出
        output_names = self.onet.get_output_names()
        cls_prob = self.onet.get_output_handle(output_names[0])
        bbox_pred = self.onet.get_output_handle(output_names[1])
        landmark_pred = self.onet.get_output_handle(output_names[2])
        cls_prob = cls_prob.copy_to_cpu()  # numpy.ndarray类型
        bbox_pred = bbox_pred.copy_to_cpu()
    ...
    def infer_image(self, img, orimg, scale=1.0):
        ...
        # print('pnet cost:{}'.format(time.time() - st))
        if boxes_c is None:
            return None, None
        # st1 = time.time()
        boxes_c = self.detect_rnet(img, boxes_c, 0.6)
        # print('rnet cost:{}'.format(time.time() - st1))
        if boxes_c is None:
            return None, None
        # st2 = time.time()
        boxes_c, landmarks = self.detect_onet(img, boxes_c, 0.7)
        # print('pnet cost:{}'.format(time.time() - st2))
        if boxes_c is None:
            return None, None
        boxes_c /= scale
        landmarks /= scale
    ...
    def det_face(self, img):
        # 用于直接检测是否存在人脸,缩放倍数0.2,确保50cm内可以检测
        # st = time.time()
